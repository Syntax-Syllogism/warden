...
import { pushErrors, readJsonOrThrow, soqlIn } from '../../userShared/sfUtils.js';
...
const addCountAction = (result: LifecycleUserResult, key: string, count: number, dryRun: boolean): void => {
  if (count === 0) return;
...
const addActivationActions = (
  ...
  if (userUpdate) {
    result.status = dryRun ? 'planned' : 'changed';
    result.actions.push(makeNotice(dryRun ? 'wouldActivate' : 'activated'));
  }
  if (loginUpdates.length > 0) {
    result.status = dryRun ? 'planned' : 'changed';
    result.actions.push(makeNotice(dryRun ? 'wouldUnfreeze' : 'unfrozen'));
  }
};
...
const addAssignmentActions = (
  ...
  plans: Pick<
    AssignmentPlan,
    'permissionSetAdds' | 'permissionSetGroupAdds' | 'publicGroupAdds' | 'queueAdds' | 'licenseAdds'
  >,
  dryRun: boolean
): void => {
  addCountAction(
  ...
  addCountAction(result, dryRun ? 'wouldAddQueueMember' : 'addedQueueMember', plans.queueAdds.length, dryRun);
  addCountAction(
    result,
...
const applyPlan = async (conn: Connection, plan: AssignmentPlan): Promise<void> => {
  ...
  if (plan.userUpdate)
    pushErrors(result.errors, await conn.sobject('User').update([plan.userUpdate], { allOrNone: false }));
  if (plan.loginUpdates.length > 0) {
    pushErrors(result.errors, await conn.sobject('UserLogin').update(plan.loginUpdates, { allOrNone: false }));
  }
  ...
  if (plan.publicGroupAdds.length > 0) {
    pushErrors(
      result.errors,
      await conn.sobject('GroupMember').create(
...
export default class UserRestore extends SfCommand<LifecycleResult> {
  ...
  public async run(): Promise<LifecycleResult> {
    ...
    const { targets, errors } = await resolveTargets(conn, requests);
    users.push(
      ...errors.map((error) => ({
        key: error.key,
    ...
    if (
      !flags['dry-run'] &&
      plans.some((plan) => plan.result.status === 'changed') &&
      !flags['no-prompt'] &&
      !this.jsonEnabled()
    ) {
