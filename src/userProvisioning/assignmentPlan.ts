...
import type { LabelMap } from '../userLifecycle/types.js';
...
export const extractCurrentIds = (
  ...
  const permissionSetGroups = assignmentRows
    ...
    .filter((value): value is string => Boolean(value));
  const publicGroups = membershipRows.filter((row) => row.Group?.Type === 'Regular').map((row) => row.GroupId);
...
export const toDmlPlan = (
  delta: AssignmentDelta,
  assignmentRows: PermissionSetAssignmentRow[],
  membershipRows: GroupMemberRow[]
): AssignmentDmlPlan => {
  const findAssignmentIds = (
    ids: string[],
    getValue: (row: PermissionSetAssignmentRow) => string | null | undefined
  ): string[] => {
