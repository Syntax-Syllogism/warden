...
export async function resolveAssignees<TGrant, TAccess extends UserAccessRow['access']>(
  ...
  const assignmentRowsByPermissionSet = await queryAllInChunks<AssignmentRow>(
    ...
    (chunk) => [...assignmentSelect, `AND PermissionSetId IN (${soqlIn(chunk)})`, 'ORDER BY Assignee.Name, Id'].join(' ')
  );
      ...
      if (parent?.Type !== 'Group') {
        ...
        if (access !== undefined) {
          ...
          const resolvedRow: UserAccessRow = {
            ...
            sourceName: isProfile ? parent?.Profile?.Name ?? parent?.Name ?? 'Profile' : parent?.Name ?? row.PermissionSetId,
            access,
          };
