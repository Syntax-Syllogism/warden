...
function runProcess(file, argv, { echo = true } = {}) {
...
// Commands in the markdown are written exactly as a user would type them
// (`sf warden ...`), so run them through the Salesforce CLI itself. That
// requires the plugin to be linked: `sf plugins link .` from the repo root.
function runCommand(command) {
  return runProcess(sfExecutable, splitArguments(command.slice('sf '.length).trim()));
}
...
// Fail once with an actionable message rather than letting every command fail
// with "command warden:... not found".
async function validatePluginLink() {
  const probe = await runProcess(sfExecutable, ['warden', '--help'], { echo: false });
  if (probe.exitCode === 0) return;
  throw new Error(
    [
      `\`${sfExecutable} warden --help\` failed with exit code ${probe.exitCode ?? 'unknown'}.`,
      `Link the plugin first: run \`sf plugins link .\` from ${repoRoot}.`,
      probe.stderr.trim(),
    ]
      .filter(Boolean)
      .join('\n')
